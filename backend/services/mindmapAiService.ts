...
export async function generateMindmap(lesson: Lesson, lessonId: string): Promise<string> {
  ...
  const prompt = buildMindmapPrompt(title, moduleNames, keyPoints, concepts, transcript, slides, mindmapCourseCtx.crossLessonBlock);
...
export async function generateMindmapModule(
  lesson: Lesson, moduleIndex: number
  ...
  const prompt = buildMindmapModulePrompt(targetTitle, targetContent);
  const result = await getModel().generateContent({
    contents: [{ role: "user", parts: [{ text: prompt }] }],
...
export async function generateMindmapNodeDetail(
  lesson: Lesson, nodeName: string, action: "explain" | "example" | "quiz" | "all"
): Promise<NodeDetailResult> {
  const transcript = (lesson.transcript || "").substring(0, 3000);
  const lessonTitle = lesson.title || "Lesson";

  const prompt = buildMindmapNodeDetailPrompt(nodeName, lessonTitle, transcript, action);
  const maxTokens = action === "all" ? 1500 : 800;
  const schema = action === "all" ? SCHEMAS.MINDMAP_NODE_ALL : null;

  const genConfig: import("@google/generative-ai").GenerationConfig = { maxOutputTokens: maxTokens };
  if (schema) {
