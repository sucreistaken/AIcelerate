// services/contextAssemblerService.ts
...
import { getCourse, getCourseForLesson } from "./courseDataService";
  ...
  let lessonBlock = "";
  {
    ...
    if (plan?.modules?.length) {
      const modSummary = plan.modules
        .slice(0, 6)
        .map((m: { title?: string; goal?: string }) => `- ${m.title || "Module"}: ${m.goal || ""}`)
        .join("\n");
      parts.push(`Modules:\n${modSummary}`);
    }
    ...
    if (emphases.length) {
      const emphSummary = emphases
        .slice(0, 8)
        .map((e: { statement: string; why?: string }) => `- ${e.statement}${e.why ? ` (${e.why})` : ""}`)
        .join("\n");
...
export async function buildToolContext(
  ...
  try {
    ...
    if (emphases.length > 0) {
      const sorted = [...emphases].sort((a: { confidence?: number }, b: { confidence?: number }) => (b.confidence || 0) - (a.confidence || 0));
    ...
    if (lesson.plan?.modules?.length) {
      const modulesSummary = lesson.plan.modules
        .map((m: { title: string; goal?: string }, i: number) => `${i + 1}. ${m.title}: ${m.goal || ""}`)
        .join("\n");
      parts.push(`=== MODULES ===\n${modulesSummary}`);
    }
    ...
    if (cs) {
      ...
      if (cs.sections?.length) {
        const sectionsText = cs.sections
          .map((s: { heading: string; bullets?: string[] }) => `## ${s.heading}\n${(s.bullets || []).map((b) => `- ${b}`).join("\n")}`)
          .join("\n\n");
      ...
      if (cs.quickQuiz?.length) {
        csParts.push(`## Quick Q&A\n${cs.quickQuiz.map((qa: { q: string; a: string }) => `Q: ${qa.q}\nA: ${qa.a}`).join("\n\n")}`);
      }
      if (csParts.length > 0) {
    ...
    if (loMods?.length) {
      ...
      for (const mod of loMods) {
        ...
        if (mod.miniQuiz?.length) {
          lines.push(`Mini Quiz:\n${mod.miniQuiz.map((mq: { question: string; answer: string }) => `  Q: ${mq.question}\n  A: ${mq.answer}`).join("\n")}`);
        }
    ...
    if (los?.length) {
      const loText = los.map((lo) => `- ${lo}`).join("\n");
      parts.push(`=== LEARNING OUTCOMES ===\n${loText}`);
    ...
    const meta: LessonContextMeta = {
      ...
      mustRememberCount: loMods?.reduce((sum: number, m: { mustRemember?: unknown[] }) => sum + (m.mustRemember?.length || 0), 0) || 0,
    ...
    logger.warn({ err, channelId, toolType }, "buildToolContext failed — returning null");
    return null;
  }
}
...
// Listen for lesson updates to invalidate caches (decoupled from lessonControllers)
eventBus.on("lesson:updated", ({ lessonId }) => {
  invalidateToolContextCache(lessonId);
});
